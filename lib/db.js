...
export async function loginEmployeeWithPassword(name, password) {
  ...
  if (existing?.length > 0) {
    ...
    // If no password set yet, set it now (migration for existing users)
    if (!existing[0].password) {
      await sb("employees", { method: "PATCH", query: "name=eq." + encodeURIComponent(name), body: { password } });
    }
  ...
  (rows || []).forEach(r => {
    if (!map[r.event_id]) {
      map[r.event_id] = {
        ...
        revenue_tickets: r.revenue_tickets || 0
      };
    }
  });
  return map;
...
export async function upsertCallLog(eventId, eventName, venue, status, empId, emp, note, noteBy, revAmt, revTix) {
  ...
  if (existing?.length > 0) {
    return await sb("call_logs", {
      method: "PATCH", query: "event_id=eq." + encodeURIComponent(eventId),
      body: { status, employee: emp || existing[0].employee, employee_id: empId || existing[0].employee_id, note: note || existing[0].note, note_by: noteBy || existing[0].note_by, revenue_amount: revAmt || existing[0].revenue_amount || 0, revenue_tickets: revTix || existing[0].revenue_tickets || 0, updated_at: new Date().toISOString() }
    });
  }
  return await sb("call_logs", {
    method: "POST",
    body: { event_id: eventId, event_name: eventName || "", venue: venue || "", status, employee: emp || "", employee_id: empId || "", note: note || "", note_by: noteBy || "", revenue_amount: revAmt || 0, revenue_tickets: revTix || 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
  });
}
...
// PHONE CACHE - with alt phone support
...
export async function cachePhone(venueKey, result, venue) {
  ...
  if (existing?.length > 0) {
    return await sb("phone_cache", {
      method: "PATCH", query: "venue_key=eq." + encodeURIComponent(venueKey),
      body: { phone: result?.phone || null, name: result?.name || venue || "", alt_phone: result?.alt_phone || null, alt_phone_label: result?.alt_phone_label || "", cached_at: new Date().toISOString() }
    });
  }
  return await sb("phone_cache", {
    method: "POST",
    body: { venue_key: venueKey, venue_name: venue || "", phone: result?.phone || null, name: result?.name || venue || "", alt_phone: result?.alt_phone || null, alt_phone_label: result?.alt_phone_label || "", cached_at: new Date().toISOString() }
  });
}
...
// SAVED EVENTS - persist loaded events per user
export async function saveEvents(events, empId, emp) {
  if (!SUPABASE_URL || !events?.length || !empId) return null;
  const batch = events.slice(0, 50).map(ev => ({
    event_id: ev.id, name: ev.name || "", date: ev.date || "", dd: ev.dd || "",
    time_str: ev.time || "", venue: ev.venue || "", addr: ev.addr || "",
    ph: ev.ph || "", alt: ev.alt || "", alt_label: ev.altL || "",
    cat: ev.cat || "", subcat: ev.subcat || "", popularity: ev.popularity || 0,
    source: ev.source || "", employee_id: empId, employee: emp || "",
    saved_at: new Date().toISOString()
  }));
  // Upsert in batches
  return await sb("saved_events", {
    method: "POST",
    query: "on_conflict=event_id,employee_id",
    body: batch,
...
export async function getSavedEvents(empId) {
  ...
  return (rows || []).map(r => ({
    id: r.event_id, name: r.name, date: r.date, dd: r.dd,
    time: r.time_str, venue: r.venue, addr: r.addr,
    ph: r.ph, alt: r.alt, altL: r.alt_label,
    cat: r.cat, subcat: r.subcat, popularity: r.popularity,
    source: r.source || "saved"
  }));
}

// Get full profile for an employee
export async function getEmployeeProfile(empId) {
  ...
  const callHistory = (logs || []).map(r => ({
    eventId: r.event_id, eventName: r.event_name, venue: r.venue,
    status: r.status, note: r.note, revenue: r.revenue_amount,
    tickets: r.revenue_tickets, timestamp: r.updated_at || r.created_at
  }));
  let totalCalls = 0, totalHits = 0, totalRevenue = 0, totalTickets = 0;
  callHistory.forEach(c => {
    totalCalls++;
    if (c.status === "HIT") totalHits++;
    totalRevenue += Number(c.revenue || 0);
    totalTickets += Number(c.tickets || 0);
  });
  return {
    callHistory, savedEvents: savedEvts?.length || 0,
    stats: { calls: totalCalls, hits: totalHits, revenue: totalRevenue, tickets: totalTickets, rate: totalCalls > 0 ? Math.round(totalHits / totalCalls * 100) : 0 }
  };
}
